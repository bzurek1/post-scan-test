    ...
    if "/in/" in blob_name:
      if operation == 'quarantine':
          generate_result_file(
              ...
              data_type = data_type,
              protecting_blob_client = protecting_blob_client
              )
          blob_name_new = blob_container + '/fynapse/' + file_type + '/malicious/' + file_name
      else:
          blob_name_new = 'fynapse/' + file_type + '/scanned/' + file_name
...
def generate_result_file(blob_name,container,data_type, protecting_blob_client: BlobServiceClient):
    container_client = protecting_blob_client.get_container_client(container)
    temp_file = blob_name.split("/")[3] + '.malicious.txt'
    f = open(temp_file, 'x')
    f.write('Content of uploaded file was detected as malicious, the file was removed.')
    f.close()
    azure_path = "fynapse/" + data_type + "/scanned/" + temp_file
    
    with open(temp_file,mode='rb' as file)
        blob_obj.create_blob_from_path(container,azure_path,file)
    os.remove(temp_file)
...
def parse_blob_information(file_url):
    ...
    data_type = blob_name.split("/")[1]
    return blob_account_name, blob_container, blob_name, data_type
